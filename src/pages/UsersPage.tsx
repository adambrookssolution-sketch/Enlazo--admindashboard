...
export function UsersPage() {
  ...
  const [editForm, setEditForm] = useState({ first_name: '', last_name_paterno: '', last_name_materno: '', display_name: '', phone: '' });
  ...
  async function loadUsers() {
    console.log('[UsersPage] loadUsers START');
    try {
      console.log('[UsersPage] fetching profiles...');
      const { data: profiles, error: profilesError } = await supabase
        .from('profiles')
        .select('id, first_name, last_name_paterno, last_name_materno, display_name, avatar_url, phone, created_at, is_blocked, blocked_reason, blocked_at')
        .order('created_at', { ascending: false });
      console.log('[UsersPage] profiles result', { count: profiles?.length, error: profilesError });

      if (profilesError) throw profilesError;

      console.log('[UsersPage] fetching user_roles...');
      const { data: allRoles, error: rolesError } = await supabase
        .from('user_roles')
        .select('user_id, role');
      console.log('[UsersPage] roles result', { count: allRoles?.length, error: rolesError });
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesSearch =
      ...
      (user.last_name_paterno?.toLowerCase().includes(q) || false) ||
  ...
  async function handleBlockUser() {
    ...
    if (!selectedUser.is_blocked && !blockReason.trim()) return;
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          last_name_paterno: editForm.last_name_paterno.trim() || null,
          last_name_materno: editForm.last_name_materno.trim() || null,
  ...
  function openEditModal(user: UserProfile) {
    ...
    setEditForm({
      ...
      last_name_paterno: user.last_name_paterno || '',
      last_name_materno: user.last_name_materno || '',
  ...
  return (
    <div>
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px', position: 'relative' }}>
                      <button
                        ...
                        style={{ padding: '8px', color: '#6B7280', backgroundColor: 'transparent', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
                        title="Editar"
                      >
                      ...
                      <button
                        ...
                        style={{ padding: '8px', color: user.is_blocked ? '#16A34A' : '#9CA3AF', backgroundColor: 'transparent', border: 'none', borderRadius: '8px', cursor: 'pointer' }}
                        title={user.is_blocked ? 'Desbloquear' : 'Bloquear'}
                      >
                        {user.is_blocked ? <CheckCircle style={{ width: '20px', height: '20px' }} /> : <Ban style={{ width: '20px', height: '20px' }} />}
                      </button>
                    </div>
                  </td>
                </tr>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: '16px', maxWidth: '400px', width: '100%', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              ...
              <button
                ...
                disabled={!selectedUser.is_blocked && !blockReason.trim()}
                style={{
                  flex: 1,
                  padding: '12px 16px',
                  border: 'none',
                  backgroundColor: selectedUser.is_blocked ? '#16A34A' : '#DC2626',
                  color: 'white',
                  fontWeight: 500,
                  borderRadius: '12px',
                  cursor: (!selectedUser.is_blocked && !blockReason.trim()) ? 'not-allowed' : 'pointer',
                  opacity: (!selectedUser.is_blocked && !blockReason.trim()) ? 0.5 : 1,
                  fontFamily: "'Centrale Sans Rounded', sans-serif"
                }}
              >
                {actionLoading ? 'Cargando...' : selectedUser.is_blocked ? 'Desbloquear' : 'Bloquear'}
              </button>
            </div>
          </div>
        </div>
      ...
      {showEditModal && selectedUser && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '16px', maxWidth: '480px', width: '100%', padding: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
              <h3 style={{ fontFamily: "'Isidora Alt Bold', sans-serif", fontSize: '18px', fontWeight: 'bold', color: '#36004E', margin: 0 }}>
                Editar Usuario
              </h3>
              <button onClick={() => { setShowEditModal(false); setSelectedUser(null); }} style={{ padding: '8px', backgroundColor: 'transparent', border: 'none', borderRadius: '8px', cursor: 'pointer' }}>
                <X style={{ width: '20px', height: '20px' }} />
              </button>
            </div>
            ...
            {[
              ...
              <div key={f.key} style={{ marginBottom: '12px' }}>
                <label style={{ display: 'block', fontSize: '14px', fontWeight: 500, color: '#374151', marginBottom: '6px', fontFamily: "'Centrale Sans Rounded', sans-serif" }}>
                  {f.label}
                </label>
                <input
                  type="text"
                  ...
                  style={{ width: '100%', padding: '10px 14px', border: '1px solid #E5E7EB', borderRadius: '10px', fontSize: '14px', fontFamily: "'Centrale Sans Rounded', sans-serif", outline: 'none', boxSizing: 'border-box' }}
                />
              </div>
            ))}

            <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
              <button onClick={() => { setShowEditModal(false); setSelectedUser(null); }} style={{ flex: 1, padding: '12px 16px', border: '1px solid #E5E7EB', backgroundColor: 'white', color: '#374151', fontWeight: 500, borderRadius: '12px', cursor: 'pointer', fontFamily: "'Centrale Sans Rounded', sans-serif" }}>
                Cancelar
              </button>
              <button
                onClick={handleEditUser}
                disabled={actionLoading || !editForm.first_name.trim()}
                style={{
                  flex: 1,
                  padding: '12px 16px',
                  border: 'none',
